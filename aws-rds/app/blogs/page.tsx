...
interface blogFilesI {
    id:string,
    title:string,
    description:string,
    tags:[]
  }
  type blogFilesType = blogFilesI[];

const BlogList = () => {
  const [blogs, setBlogs] = useState<blogFilesType>([]);
          ...
          {blogs.map((blog) => ( 
            <li key={blog.id} className="bg-[#001313] p-8 rounded-lg shadow-sm shadow-green-300 md:px-8 md:py-8 hover:shadow-green-300">
              <div className="flex justify-between items-center ">
                <Link href={`/blog/${blog.id}`} className="text-gray-100 hover:text-green-400 text-lg">
                  {blog.title}
                </Link>
